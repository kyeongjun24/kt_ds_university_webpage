	...
	function getCheckList() {
		...
		if (length == 0) {
			...
			$
					.ajax({
						type : 'post',
						url : '${contextPath}/survey/removeCheckedSurveys.do',
						...
						success : function(data) {
							alert('삭제 완료');
							window.location.href = "${contextPath}/survey/listSurveys.do";
						},
				...
				$('#listFilter')
						.on(
								...
								function() {
									...
									alert(searchType);
									alert(searchText); */
									location.href = "${contextPath}/survey/listSurveys.do?perPage="
											+ perPage
											+ "&searchType="
											+ searchType
											+ "&searchText="
...
<body>
	...
	%>
	<form method="get" action="${contextPath}/survey/listSurveys.do" id="searchFrm">
		...
		<ul>
			<c:if test="${pageMaker.prev }">
				<c:choose>
					<c:when test="${not empty searchType and not empty searchText }">
						<li><a
							href="${contextPath}/survey/listSurveys.do?page=${pageMaker.startPage - 1 }&searchText=${searchText}&searchType=${searchType}">이전</a></li>
					</c:when>
					<c:otherwise>
						<li><a
							href="${contextPath}/survey/listSurveys.do?page=${pageMaker.startPage - 1 }&searchText=${searchText}&searchType=${searchType}">이전</a></li>
				...
				<c:when test="${not empty searchType and not empty searchText }">
					<c:forEach begin="${pageMaker.startPage }"
						...
						<li
							...
							<a
							href="${contextPath }/survey/listSurveys.do?page=${idx}&searchText=${searchText}&searchType=${searchType}&perPage=${perPage}">${idx }</a>
						</li>
					</c:forEach>
				</c:when>
				<c:otherwise>
			...
			<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
				<c:choose>
					<c:when test="${not empty searchType and not empty searchText }">
						<li><a
							href="${contextPath}/survey/listSurveys.do?page=${pageMaker.endPage + 1 }&searchText=${searchText}&searchType=${searchType}">다음</a></li>
					</c:when>
					<c:otherwise>
						<li><a
							href="${contextPath}/survey/listSurveys.do?page=${pageMaker.endPage + 1 }&searchText=${searchText}&searchType=${searchType}">다음</a></li>
